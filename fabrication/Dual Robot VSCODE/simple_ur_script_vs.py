...
def move_j(joints, accel, vel, time, blendradius):
    ...
    Args:
        ...
        accel: tool accel in m/s^2
        vel: tool speed in m/s
        time: time to finish the move in s
        blendradius: blend radius through the whole movement in m
    ...
    script = "movej(%s, a = %.3f, v = %.3f, t = %s, r = %s)\n"%(_j_fmt,accel,vel,time,blendradius)
    #script = "movej(%s, a = %.2f, v = %.2f)\n"%(_j_fmt,accel,vel)
    return script

def set_tcp_by_plane(x_offset, y_offset, z_offset, ref_plane):
    """
